...
function update(updatReview) {
    var reviewIdx = reviews.findIndex(review => review._id === updatReview._id);
...
function getById(id) {
    var review = reviews.find(review => review._id === id);
    if (review) return Promise.resolve(review);
    else return Promise.reject('Unknown review');
}

function remove(reviewId) {
    var reviewIdx = reviews.findIndex(review => review._id === reviewId);
